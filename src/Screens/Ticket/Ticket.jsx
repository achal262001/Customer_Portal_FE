...
import { mockTickets } from "./constant";
...
const DetailRow = ({ label, children }) => (
  <Box sx={{ display: "flex", gap: 2, py: 1.5, alignItems: "flex-start" }}>
    <Typography variant="body2" sx={{ color: "#9ca3af", minWidth: 110, fontWeight: 500 }}>
      {label}
    </Typography>
...
const Ticket = () => {
  ...
  const handleNewTicket = (ticket) => {
    setTickets((prev) => {
      ...
      return [{ ...ticket, id: newId, attachments: [] }, ...prev];
    });
  };
      ...
      >
        ...
        >
          {["", "ID", "Title", "Category", "Priority", "Status", "Created", ""].map((h) => (
            <Typography
              ...
              variant="caption"
              fontWeight={700}
              sx={{ color: "#6b7280", textTransform: "uppercase", letterSpacing: 0.5, fontSize: "0.7rem" }}
            >
              {h}
            </Typography>
          ))}
        ...
        {filtered.length > 0 ? (
          filtered.map((ticket, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* ID */}
                <Typography variant="body2" fontWeight={600} sx={{ color: "#374151", fontSize: 13 }}>
                  {ticket.id}
                </Typography>
                ...
                {/* Category */}
                <Typography variant="body2" sx={{ color: "#6b7280", textTransform: "capitalize", fontSize: 13 }}>
          ...
          <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", py: 10 }}>
            <ConfirmationNumberOutlinedIcon sx={{ fontSize: 56, color: "#d1d5db", mb: 1.5 }} />
      ...
      >
        {selectedTicket && (
          <DialogContent sx={{ p: 0 }}>
            ...
            >
              <Box>
                <Typography variant="caption" fontWeight={700} sx={{ color: "#9ca3af", letterSpacing: 0.6 }}>
                  {selectedTicket.id}
                </Typography>
                <Typography variant="h6" fontWeight={700} sx={{ color: "#111827", mt: 0.3, lineHeight: 1.3 }}>
                  {selectedTicket.title}
                </Typography>
              </Box>
              <IconButton size="small" onClick={() => setSelectedTicket(null)} sx={{ mt: -0.5 }}>
            ...
            <Box sx={{ px: 3, pt: 1, pb: 3 }}>
              {/* Description */}
              <Box sx={{ py: 2 }}>
                <Typography variant="caption" fontWeight={700} sx={{ color: "#9ca3af", textTransform: "uppercase", letterSpacing: 0.6 }}>
                  Description
                </Typography>
                <Typography variant="body2" sx={{ color: "#374151", mt: 1, lineHeight: 1.8 }}>
              ...
              <Box sx={{ mt: 1 }}>
                <DetailRow label="Status">
                  <Chip
                    ...
                    size="small"
                    sx={{
                      backgroundColor: statusColors[selectedTicket.status]?.bg || "#f5f5f5",
                      color: statusColors[selectedTicket.status]?.text || "#555",
                      fontWeight: 600,
                      textTransform: "capitalize",
                      fontSize: "0.75rem",
                      borderRadius: 1.5,
                ...
                <DetailRow label="Priority">
                  <Chip
                    ...
                    size="small"
                    sx={{
                      backgroundColor: priorityColors[selectedTicket.priority]?.bg || "#f5f5f5",
                      color: priorityColors[selectedTicket.priority]?.text || "#555",
                      fontWeight: 600,
                      textTransform: "capitalize",
                      fontSize: "0.75rem",
                      borderRadius: 1.5,
